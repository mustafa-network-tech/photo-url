...
const slug = value => GalleryModel.fold(value).replace(/[^a-z0-9]+/g, '-').replace(/^-|-$/g, '');
module.exports = function generate(root, out, items) {
  ...
  const route = category => '/kategori/'+categories.find(x=>x.collection===category.collection&&x.category===category.category).slug+'/';
  ...
  const group = g => `<section class="featured-group"><div class="group-heading"><h3>${esc(g.label)}</h3><a class="view-all" href="${route(g)}">Tümünü gör →</a></div><div class="gallery">${g.items.map(card).join('')}</div></section>`;
  function page(category) {
    const selected = category ? items.filter(x=>x.collection===category.collection&&x.category===category.category) : items;
    const pathname = category ? route(category) : '/';
    const title = category ? category.category+' Fotoğrafları | Mavi Kadraj Fotoğraf Arşivi' : 'Mavi Kadraj Fotoğraf Arşivi | Konular ve Şehirler';
    const description = category ? `Mavi Kadraj Fotoğraf Arşivi’nin ${category.category} kategorisindeki ${selected.length} fotoğrafı keşfedin ve fotoğraf bağlantılarına ulaşın.` : 'Mavi Kadraj Fotoğraf Arşivi’nde konu ve şehir kategorilerindeki fotoğrafları keşfedin, önizlemeleri inceleyin ve fotoğraf bağlantılarını kopyalayın.';
    const graph = [{'@type':'CollectionPage','@id':origin+pathname,name:title,description,url:origin+pathname,isPartOf:{'@id':origin+'/#website'}}];
    if(category) graph.push({'@type':'BreadcrumbList',itemListElement:[{'@type':'ListItem',position:1,name:'Fotoğraf Arşivi',item:origin+'/'},{'@type':'ListItem',position:2,name:category.category,item:origin+pathname}]});
    else graph.push({'@type':'WebSite','@id':origin+'/#website',name:'Mavi Kadraj Fotoğraf Arşivi',url:origin+'/'});
    ...
    html = html.replace('id="photo-count">0','id="photo-count">'+selected.length).replace('id="category-count">0','id="category-count">'+(category?1:categories.length));
    html = html.replace('<div id="gallery"></div>',`<div id="gallery">${category?'<div class="gallery">'+selected.map(card).join('')+'</div>':GalleryModel.featured(items).map(group).join('')}</div>`);
    ...
    html = html.replace('</main>',`<nav aria-label="Fotoğraf kategorileri"><p>${categories.map(c=>`<a href="${route(c)}">${esc(c.category)}</a>`).join(' · ')}</p></nav></main>`);
    if(category) html = html.replace('<h1>Mavi Kadraj <span>Fotoğraf Arşivi</span></h1>',`<h1>${esc(category.category)} <span>Fotoğrafları</span></h1>`).replace('<p class="intro">Kadrajdan arşive, yılların biriktirdiği fotoğraflar.</p>',`<p class="intro"><a href="/">Fotoğraf Arşivi</a> › ${esc(category.category)}</p>`).replace('</head>',`<script>window.GALLERY_CATEGORY=${JSON.stringify(category).replace(/</g,'\\u003c')};</script></head>`);
    const dest = path.join(out,category?'kategori/'+category.slug+'/index.html':'index.html');
    fs.mkdirSync(path.dirname(dest),{recursive:true});fs.writeFileSync(dest,html);
  }
  page();categories.forEach(page);
  const entries = [`<url><loc>${origin}/</loc></url>`,...categories.map(c=>`<url><loc>${origin+route(c)}</loc>${items.filter(x=>x.collection===c.collection&&x.category===c.category).map(x=>`<image:image><image:loc>${esc(url(x.path))}</image:loc></image:image>`).join('')}</url>`)];

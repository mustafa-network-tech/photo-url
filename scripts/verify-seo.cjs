...
(async()=>{
 ...
 const pages=[...xml.matchAll(/<loc>(.*?)<\/loc>/g)].map(x=>x[1]);
 const photos=[...xml.matchAll(/<image:loc>(.*?)<\/image:loc>/g)].map(x=>x[1].replace(/&amp;/g,'&'));
 assert.equal(pages.length,inventory.categories.length+1);assert.equal(new Set(photos).size,built.length);
 ...
 assert.equal((await fetch(base+'/kategori/bulunmayan/')).status,404);
 ...
 fs.writeFileSync('SEO-TEST-RESULTS.json',JSON.stringify(report,null,2));console.log(JSON.stringify(report,null,2));
